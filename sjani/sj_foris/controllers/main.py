...
models = {
    ...
    'article': {
        ...
        'model': 'article.article_sj',
    },
    'publishment': {
        'fields': ['id', 'title', 'article_pdf', 'publishment_date', 'image'],
        'fields_pagination': ['id', 'title', 'article_pdf', 'publishment_date', 'image'],
        'model': 'publishment.publishment_sj',
        'filter_by': {'title': 'ilike'},
        'display_fields': ['id', 'title']
    },
    'translation': {
        'fields': ['id', 'title', 'translation_id', 'article_pdf', 'publishment_date', 'image'],
        'fields_pagination': ['id', 'title', 'translation_id', 'article_pdf', 'publishment_date', 'image'],
        'model': 'translation.translation_sj',
        'filter_by': {'title': 'ilike'},
        'display_fields': ['id', 'title']
    },
    'talents': {
        ...
        'model': 'talents.talents_sj',
        'filter_by': {},
    },
    'application': {
        'fields': ['id', 'name', 'surname', 'email', 'job', 'institution', 'phone_number', 'content', 'upload_file',
                   'nr_amzes', 'nr_rendor', 'school_name', 'professor_name', 'professor_name'],
        'model': 'application.application_sj',
...
class ForisAPI(http.Controller):
    ...
    # API FOR LEGAL ADIVCE FORM
    @http.route(['/api/create_legal_advice', ], auth="public", type="json", methods=['POST'], csrf=False, cors="*")
    def create_legal_advice(self):
        try:
            request_data = request.jsonrequest
            record = request.env['legal.advice_sj'].sudo().create(request_data)
            Response.status = '200'
            return {'id': record.id}
        except Exception as error:
            request._cr.rollback()
            Response.status = '400'
            return {'message': 'Ndodhi nje problem ne krijim! {error}'.format(error=error if error else '')}

        # API FOR APPLICATION FORM
